...
A Hermes skill that finds jobs that match you, writes a weekly digest, and can tailor a LaTeX CV. Hermes sends the digest to the chat apps you already use.

## Prerequisites

- **[Hermes Agent](https://hermes-agent.nousresearch.com/)** — desktop app or terminal install
- **[Docker](https://docs.docker.com/get-docker/)** — needed to compile tailored CV PDFs
- **A messaging channel** — Discord, Telegram, Slack, or another Hermes gateway. Run `hermes gateway setup` (or `hermes setup`) and connect one so the digest can reach you.
...
**3. Get it every Monday (optional).** Installing the skill suggests a Monday 09:00 job. Accept it from `/suggestions` when you want a weekly run. The job delivers the digest to every connected home channel.

## Further configuration

Defaults are enough for the steps above. Change them only if you need to.

**Where files live.** Your data goes under `~/listen-for-work` (`input/`, `output/`, `job-descriptions/`, `digest/`). To use another folder:
...
**Tailor one job.** Ask Hermes to adapt your CV to a company or a job-description file. Docker must be running.

**CV PDFs in chat.** If the files do not attach, add `~/listen-for-work/output` to `HERMES_MEDIA_ALLOW_DIRS`.

**Monday delivery.** Default `deliver` is `all`. Point it at one channel with `hermes cron edit` (`discord:#jobs`, `telegram`, `origin`).
